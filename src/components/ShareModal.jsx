...
const ShareModal = ({ isOpen, onClose, product }) => {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      action: () => {
        const message = `${productTitle}\n${productDescription}\n${productUrl}`;
        window.open(`https://t.me/share/url?url=${encodeURIComponent(productUrl)}&text=${encodeURIComponent(message)}`, '_blank');
      }
    },
    {
      name: 'Email',
          ...
          <div className="bg-gray-700 rounded-xl p-4">
            ...
            <div className="flex items-center space-x-3">
              <input
                ...
                value={productUrl}
                readOnly
                className="flex-1 bg-gray-600 text-white text-sm px-3 py-2 rounded-lg border border-gray-500 focus:outline-none focus:border-gold"
              />
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                  copied
                    ...
                    : 'bg-gradient-to-r from-gold to-yellow-500 text-black hover:from-yellow-500 hover:to-gold hover:shadow-lg'
                }`}
              >
                <CopyIcon />
              ...
              {shareOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`group flex flex-col items-center justify-center p-4 bg-gradient-to-br ${option.color} ${option.hoverColor} text-white rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg`}
                  >
                    <IconComponent className="text-2xl mb-2 group-hover:scale-110 transition-transform" />
