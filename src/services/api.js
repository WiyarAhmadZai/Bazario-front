...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (!error.response) {
      ...
      console.error(
        "Network error: No response from server. Please check your connection and ensure the server is running."
      );
      throw new Error(
    ...
    // Only logout automatically on 401 if it's not related to specific endpoints
    if (error.response?.status === 401) {
      // Skip logout for specific endpoints that might return 401 for business logic
